...
class ImageProcessor:
    ...
    def create_polar_histogram(self, frame: np.ndarray, ax) -> np.ndarray:
        """Create polar histogram from frame"""
        ax.clear()
        ...
        if height != width:  # CORTA A IMAGEM PARA FICAR QUADRADO
            start_x = (width - size) // 2
            start_y = (height - size) // 2
            img_copy = img_copy[start_y:start_y+size, start_x:start_x+size]   
        cv2.imshow('img_copy',img_copy)
        gray = cv2.cvtColor(img_copy, cv2.COLOR_BGR2GRAY)                                               # PRÉ PROCESSA REMOVENDO CANAIS DE CORES RGB
        
        _, black_mask = cv2.threshold(gray, self.params.threshold_value, 255, cv2.THRESH_BINARY_INV)    # USA THRESHOLD PARA CORES ÚNICAS
        
        center_x, center_y = size // 2, size // 2                                                       # DETERMINA QUE O CENTRO DO GRÁFICO SERÁ NO CENTRO DA IMAGEM
        max_radius = size // 2                                                                          # DETERMINA QUE O RAIO DO GRÁFICO TOCARÁ A BORDA DO GRÁFICO
        
        angle_step = 360 / self.params.num_bins
        angles = np.arange(0, 360, angle_step)                                                          # CRIA UMA SEQUÊNCIA NUMÉRICA DE INTERVALO (INÍCIO, FIM, PASSO) EM FORMATO DE NUMPY ARRAY
        
        sector_black_counts = np.zeros(self.params.num_bins)                                            # CRIA UMA LISTA DE ZEROS DO TAMANHO DO INTEIRO
        sector_total_pixels = np.zeros(self.params.num_bins)
        sector_black_percentages = np.zeros(self.params.num_bins)
        
        y_coords, x_coords = np.indices((size, size))                                                   # SEI LÁ NÃO LEMBRO
        
        dx = x_coords - center_x                                                                        # SUBTRAI A DISTÂNCIA DO PIXEL DA IMAGEM DO CENTRO DA IMAGEM
        dy = y_coords - center_y
        
        angles_rad = np.arctan2(dy, dx)                                                                 # ARCOTANGENTE DE TODOS OS PARES DE COORDENADAS
        angles_deg = np.degrees(angles_rad)                                                             # CONVERTE E TAL
        ...
        theta_rose = np.radians(np.append(angles, angles[0]))                                   #### AQUI É ONDE FICAM TODAS AS COORDENADAS DOS PONTOS
        values_rose = np.append(sector_black_percentages, sector_black_percentages[0])
        #m = 0
        #for i in zip(values_rose,theta_rose):
        #    print(m, i)
        #    m+=1
        
        ax.fill(theta_rose, values_rose, alpha=0.5, color='black')
        ax.plot(theta_rose, values_rose, color='black', linewidth=1)
        
        ax.set_theta_zero_location('E')
        ax.set_theta_direction(-1)
        
        ax.set_ylim(0, max(values_rose)//2 + ((max(values_rose) // 2) * 0.2))
        ax.set_title(f'polar')
    ...
    def crop_ring_area(image: np.ndarray, center_x: int, center_y: int,
        ...
        """Crop ring area between inner and outer radius"""
        hh, ww = image.shape[:2]
        ...
        return final_image[y1:y2, x1:x2]
...
class RingProcessor:
    """Process the ring area of the gauge"""
    
    def __init__(self, params: ProcessingParams):
        self.params = params
        self.processor = ImageProcessor(params)
        self.image_processor = ImageProcessor(params)
    ...
    def process(self, ring_img: np.ndarray, n_regions: int) -> np.ndarray:
        """Process ring image and detect regions"""
        gray = self.processor.convert_to_gray(ring_img)
        
        _, output_thresh = cv2.threshold(gray, self.params.ring_thresh_low,self.params.ring_thresh_high, cv2.THRESH_BINARY)
        #_, output_thresh2 = cv2.threshold(gray, self.params.ring_thresh_low,self.params.ring_thresh_high, cv2.THRESH_BINARY_INV)
        
        mask_for_cc = 255 - output_thresh
        #mask_for_cc2 = 255 - output_thresh2
        #gray2 = cv2.cvtColor(mask_for_cc2, cv2.COLOR_BGR2GRAY)
        
        self.image_processor.create_polar_histogram(ring_img, self.ax)
        self.fig.canvas.draw()
        self.fig.canvas.flush_events()
        
        regions = self.processor.find_black_regions(mask_for_cc, self.params.min_region_area)
        
        output_color = cv2.cvtColor(output_thresh, cv2.COLOR_GRAY2BGR)
        
        for i, region in enumerate(regions[:n_regions]):
            cv2.rectangle(output_color,(region['x'], region['y']),(region['x'] + region['width'], region['y'] + region['height']),(0, 0, 255), 1)
            cv2.putText(output_color, str(i+1),(region['centroid'][0], region['centroid'][1]),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
...
class NeedleDetector:
    """Detect gauge needle using background subtraction and line fitting"""
    ...
    def __init__(self, params: ProcessingParams):
        ...
        self.ring_processor = RingProcessor(params)
        self.image_processor = ImageProcessor(params)
    ...
    def process_ring(self, frame: np.ndarray, circle_data: Tuple[int, int, int],
                      n_regions: int):
        """Process the ring area of the gauge"""
        x, y, r = circle_data
        ...
        if inner_radius < r:
            ring_cropped = self.image_processor.crop_ring_area(frame, x, y, inner_radius, r)
...
class GaugeReaderApp:
    ...
    def __init__(self, video_path: str):
        ...
        self.image_processor = ImageProcessor(self.processing_params)
    ...
    def setup_windows(self):
        ...
        cv2.createTrackbar('thresh_value',      'CONTROLS', self.processing_params.thresh_value,    1500,   lambda v: setattr(self.processing_params, 'thresh_value', v))
        cv2.createTrackbar('thresh_value2',     'CONTROLS', self.processing_params.thresh_value2,   1500,   lambda v: setattr(self.processing_params, 'thresh_value2', v))
        cv2.createTrackbar('H_varThreshold',    'CONTROLS', self.processing_params.var_threshold,   650,    lambda v: setattr(self.processing_params, 'var_threshold', v))
    ...
    def process_frame(self, frame: np.ndarray) -> Optional[np.ndarray]:
        ...
        if dist > 0:
            ...
            self.visualizer.draw_intersection_marker(
                black_foreground, (intersection_x, intersection_y)
            )
            
            current_angle = self.angle_calculator.calculate_needle_angle(
                (x, y), extended_point
            )
    ...
    def run(self):
        ...
        #print("\n" + "="*50)
        #print("GAUGE READER APPLICATION")
        #print("="*50)
        #print("\nControls:")
        #print("  SPACE - register current position")
        #print("  c - calculate all angles")
        #print("  v - print results")
        #print("  s - reset ROI to default")
        ...
        while True:
            ...
            if result_frame is not None: cv2.imshow("SCENE", result_frame)
