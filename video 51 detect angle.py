...
class ImageProcessor:
    ...
    def create_polar_histogram(self, frame: np.ndarray, ax, gauge_center: Optional[Tuple[int, int]] = None, scale_factor: float = 1.0, target_img: Optional[np.ndarray] = None, result_frame: Optional[np.ndarray] = None, offset: Optional[Tuple[int, int]] = None):
        ...
        if len(connected_results) > 0:
            ...
            for i, region in enumerate(connected_results[:50]):
        ...
        if self.app and hasattr(self.app, 'current_result_frame'):
            ...
            if result_frame is not None and offset is not None and hasattr(self.app, 'circle_data'):
                ...
                for i, (x, y) in enumerate(component_centroid):
                    ...
                    cv2.line(result_frame,      (center_x, center_y), (original_x, original_y), (90, 90 ,90), 1)
                    cv2.line(frame,             (center_x, center_y),(original_x, original_y), (0, 0, 125), 1)
                    ...
                    if dist > 0:
                        ...
                        extended_x = int(center_x + dx_norm * (radius + 3 * radius))
                        extended_y = int(center_y + dy_norm * (radius + 3 * radius))

                        intersections = self.angle_calculator.find_line_circle_intersections((center_x, center_y),
                        ...
                        if self.app and hasattr(self.app, 'min_line'):                                                                  ########### AQUI A GENTE HERDA X0,Y0 E X1,Y1 DA LINHA MÍNIMA E MÁXIMA
                            ...
                            min_lineB, min_lineA = min_line
                            min_line = (min_lineA, min_lineB)
                            #print(min_lineA, min_lineB)
                            cv2.circle(result_frame, min_line[1], 6, (0, 255, 255), 1)                         ##### CÍRCULO QUE MOSTRA A INTERSECÇÃO DA MARCA DA ESCALA E A BORDA
                        ...
                        if self.app and hasattr(self.app, 'max_line'):                                                                  ########### AQUI A GENTE HERDA X0,Y0 E X1,Y1 DA LINHA MÍNIMA E MÁXIMA
                            ...
                            min_scale = 0
                            max_scale = self.angle_calculator.directed_angle(max_line,min_line) + 180
                            actual_scale = self.angle_calculator.directed_angle(((center_x, center_y), (original_x,original_y)),min_line)
                                
                            if (min_scale + 3 <= actual_scale) and (actual_scale <= max_scale):
                                
                                cv2.line(result_frame, (center_x, center_y), (original_x, original_y), (200,200,200), 1)
                                ...
                                if dist > 0:
                                    ...
                                    circle_edge_x, circle_edge_y = int(center_x + dx_norm * radius), int(center_y + dy_norm * radius)
                                    
                                    extension_beyond = 15           
                                    extended_x, extended_y = int(circle_edge_x + dx_norm * extension_beyond), int(circle_edge_y + dy_norm * extension_beyond)

                                    cv2.line(result_frame, (center_x, center_y), (extended_x, extended_y), (200, 200, 200), 1)      ######### DESENHA A LINHA CINZA-CLARO
                                    cv2.circle(result_frame, (circle_edge_x, circle_edge_y), 3, (255, 255, 0), -1)                  ###########DESENHA A INTERSECÇÃO EM AZUL-CLARO
                                    cv2.putText(result_frame, str(int(actual_scale)), (original_x,original_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

                                if not hasattr(self.app, 'tick_mark_angles'):
                                    self.app.tick_mark_angles = []
                                
                                if actual_scale not in self.app.tick_mark_angles:
                                    self.app.tick_mark_angles.append(actual_scale)
                                    self.app.tick_mark_angles.sort()
...
class AngleCalculator:
    ...
    def directed_angle(line1: Tuple[Tuple[int, int], Tuple[int, int]], 
        ...
        Uses cross product to determine direction without explicit angle calculation
        """
        center1, point1 = line1
        center2, point2 = line2
...
class GaugeReaderApp:
    def __init__(self, video_path: str):
        ...
        self.tick_mark_angles = [] 
        self.crossed_angles = set()  
            ...
            if points:
                ...
                if result:
                    ...
                    if dist > 0:
                        ...
                        if hasattr(self, 'tick_mark_angles') and self.tick_mark_angles:
                            for tick_angle in self.tick_mark_angles:
                                
                                if current_angle > tick_angle and tick_angle not in self.crossed_angles:
                                    
                                    print(f"Yellow line {int(tick_angle)}°")
                                    self.crossed_angles.add(tick_angle)
