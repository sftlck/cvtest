            ...
            if circle_detected and circle_data and needle_origin and needle_end:
                ...
                if length > 0:
                    ...
                    text_items = [
                        ...
                        ("list_positions",          f"{len(list_positions)}",                   "",         50),

                        ("current_angle",           f"{current_angle_deg_ref:.2f}",             "deg",      110),
                        ("circle_data",             f"({circle_data[0]}, {circle_data[1]})",    "pixels",   130),
                        ("relative_angle",          f"{round(current_angle,2)}",                "deg",      150),
                        ("TESTE_relative_value",    f"{round(teste_relative_value,2)}",         "mm",       170),
                        ("current_len(fitline)",             f"{len(fitline)}",                          "",         190)
                    ]
...
def calculate_all_angles():
    ...
    if len(list_positions) < 2:
        print("Need at least 2 registered positions to calculate angles")
        return []
