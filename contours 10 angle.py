...
def detect_lines(frame):
            ...
            if lines is not None:
                ...
                for line in lines:
                    ...
                    if dist1 <= r and dist2 <= r:
                        ...
                        line_coordinates_list.append(line_coords) 
                        circle_points_for_fitting.append([x1, y1])
                        
                        #cv2.imshow('LSD', frame)
                
                if len(circle_points_for_fitting) >= 2:
                    points_array = np.array(circle_points_for_fitting, dtype=np.float32)
                    ...
                    if abs(vx) > 1e-6:
                        ...
                        left_y = int(y0 + slope * (left_x - x0))
                        right_y = int(y0 + slope * (right_x - x0))
                        
                        #cv2.line(frame, (left_x, left_y), (right_x, right_y), (255, 0, 0), 2)##LINHA APENAS COM INICIAL E FINAL
    ...
    if len(all_points_for_fitting) >= 2:
        ...
        if abs(vx_all[0]) > 1e-6:
            ...
            needle_origin = (circle_info[0], circle_info[1])  # Circle center
            ...
            if circle_detected and circle_info and needle_origin and needle_end:
                ...
                if length > 0:
                    ...
                    if distance2 > 0:
                        ...
                        cv2.drawMarker(black_foreground, yellow_line_intersection,(0,0,255),cv2.MARKER_SQUARE,20,1,1)####INTERSECCÇÃO EM BLACK_FOREGROUND
                    ...
                    cv2.putText(black_foreground, angle_text, (10, height-30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
