...
############# FUNCIONAL PARA MOSTRADORES ANALÓGICOS DE ESCALA COM NÚMEROS NATURAIS - FORMATO VÍDEO

############# CONSTANTES PARA GOODFEATURESTOTRACK
...
def setup_undistort_maps(camera_matrix, dist_coeffs, width, height):
    ...
    map1, map2 = cv2.initUndistortRectifyMap(camera_matrix, dist_coeffs, None, new_camera_matrix, (width, height), cv2.CV_16SC2)
...
def select_roi_window():
    ...
    while True:
        ...
        if roi_selector.circle:
            ...
            display_frame = roi_selector.draw_circle(cropped.copy())
...
def detect_lines(frame):
    ...
    if circle_detected and circle_data:
        ...
        # Get trackbar values
        maxCorners = cv2.getTrackbarPos('Max Corners', goodfeaturestotrack)
        qualityLevel_val = cv2.getTrackbarPos('Quality Level (x100)', goodfeaturestotrack)
        minDistance = cv2.getTrackbarPos('Min Distance', goodfeaturestotrack)
        blockSize = cv2.getTrackbarPos('Block Size', goodfeaturestotrack)
        ...
        # Create a circular mask for goodFeaturesToTrack
        circle_mask = np.zeros(gray2.shape[:2], dtype=np.uint8)
        cv2.circle(circle_mask, (x, y), circle_tolerance, 255, -1)
        
        # Apply goodFeaturesToTrack only within the circular mask
        corners = cv2.goodFeaturesToTrack(gray2, 
                                        maxCorners=maxCorners,
                                        qualityLevel=qualityLevel,
                                        minDistance=minDistance,
                                        blockSize=blockSize,
                                        mask=circle_mask)  # Add the mask parameter here
        ...
        if corners is not None:
            ...
            for corner in corners:
                ...
                cv2.circle(gray2, (x_corner, y_corner), 3, (0, 0, 255), -1)
        
        cv2.imshow(goodfeaturestotrack, gray2)

        # Rest of your contour processing code...
        ...
        if len(all_points_for_fitting) >= 20:
            ...
            if abs(vx_all[0]) > 1e-6:
                ...
                centroid = (int(x0_all[0]), int(y0_all[0]))
                    ...
                    if length > 0:
                        ...
                        if distance2 > 0:
                            ...
                            for i, (reg_circle_center, reg_extended_point) in enumerate(list_positions):
                                ...
                                cv2.putText(black_foreground, str(i+1), (reg_extended_point[0] + 10, reg_extended_point[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
...
if __name__ == "__main__":
    ...
    cv2.namedWindow(goodfeaturestotrack)

    cv2.createTrackbar('Max Corners',           goodfeaturestotrack, maxCorners,       500,    nothing)
    cv2.createTrackbar('Quality Level (x100)',  goodfeaturestotrack, qualityLevel,     100,    nothing)
    cv2.createTrackbar('Min Distance',          goodfeaturestotrack, minDistance,      50,     nothing)
    cv2.createTrackbar('Block Size',            goodfeaturestotrack, blockSize,        31,     nothing)
    ...
    while True:
        ...
        progress = f"Frame: {current_frame}/{total_frames} ({(current_frame/total_frames*100):.1f}%)"
        ...
        elif k == ord('p'):    # Pause/Resume video
            pause_video = not pause_video
            print(f"Video {'paused' if pause_video else 'resumed'}")
        elif k == ord('r'):    # Reset video to beginning
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
