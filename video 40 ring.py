...
class ImageProcessor:
    ...
    def create_polar_histogram(self, frame: np.ndarray, ax, gauge_center: Optional[Tuple[int, int]] = None, scale_factor: float = 4.0, target_img: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if target_img is not None:
            for val, ang in zip(values_rose, theta_rose):
                x_img = int(center_x + val * np.cos(ang) * scale_factor)
                y_img = int(center_y + val * np.sin(ang) * scale_factor)
                if 0 <= x_img < target_img.shape[1] and 0 <= y_img < target_img.shape[0]:
                    cv2.circle(target_img, (x_img, y_img), 2, (0, 255, 0), -1)
            cv2.circle(target_img, (center_x, center_y), 3, (0, 0, 255), -1)
    ...
    @staticmethod
    def crop_ring_area(image: np.ndarray, center_x: int, center_y: int, inner_r: int, outer_r: int) -> np.ndarray:
